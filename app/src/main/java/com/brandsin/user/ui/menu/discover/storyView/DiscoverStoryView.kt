...
class DiscoverStoryView(
    ...
    private lateinit var pagerAdapter: DiscoverStoryPagerAdapter
    ...
    private var discoverStoryViewListener: DiscoverStoryViewListener? = null
    ...
    fun setStoryViewListener(discoverStoryViewListener: DiscoverStoryViewListener?) {
        this.discoverStoryViewListener = discoverStoryViewListener
    }

    interface DiscoverStoryViewListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (discoverStoryViewListener == null) {
    ...
    private fun setBtnListener() {
        ...
        binding.imgStore.setOnClickListener {
            ...
            dismiss()
            dismissAllowingStateLoss()
            discoverStoryViewListener?.onDoneClicked(
                num,
                viewModel.storyItem!!
            )
        }
    ...
    private fun setUpPager() {
        pagerAdapter = DiscoverStoryPagerAdapter(
            this,
    ...
    override fun nextPageView() {
        if (binding.viewPagerFixedViewPager.currentItem + 1 <
            (binding.viewPagerFixedViewPager.adapter?.count ?: 0)
        ) {
            try {
                fakeDrag(true)
            } catch (e: Exception) {
    ...
    private fun currentFragment(): DiscoverStoryDisplayFragment {
        return pagerAdapter.findFragmentByPosition(
            binding.viewPagerFixedViewPager,
            currentPage
        ) as DiscoverStoryDisplayFragment
    }
