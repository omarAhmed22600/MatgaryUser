...
class AddAddressActivity : AppCompatActivity(), Observer<Any?>, OnMapReadyCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (PrefMethods.getUserData()!!.phoneNumber != null) {
            viewModel.addAddressRequest.phoneNumber =
                PrefMethods.getUserData()!!.phoneNumber.toString()
        }
        ...
        observe(viewModel.isMapReady) {
            when (it) {
                true -> {
                    val latLng = LatLng(
                        viewModel.addAddressRequest.lat!!.toDouble(),
                        viewModel.addAddressRequest.lng!!.toDouble()
                    )
    ...
    override fun onChanged(it: Any?) {
        ...
        when (it) {
            ...
            Codes.ADDRESS_SAVED -> {
                val addedAddress = AddedAddress(
                    viewModel.newAddressResponse.id,
                    viewModel.newAddressResponse.lat,
                    viewModel.newAddressResponse.lng,
                    viewModel.newAddressResponse.streetName,
                    viewModel.newAddressResponse.addressName,
                    viewModel.newAddressResponse.type,
                    viewModel.newAddressResponse.typeLabel,
                    viewModel.newAddressResponse.phoneNumber,
                ...
                when (data) {
                    null -> {
                        ...
                        when (viewModel.addAddressRequest.lat) {
                            null -> {
                                viewModel.addAddressRequest.lat =
                                    PrefMethods.getUserLocation()!!.lat
                                viewModel.addAddressRequest.lng =
                                    PrefMethods.getUserLocation()!!.lng
                                viewModel.addAddressRequest.streetName =
