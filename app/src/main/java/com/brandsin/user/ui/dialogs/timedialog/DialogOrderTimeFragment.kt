...
class DialogOrderTimeFragment : DialogFragment(), Observer<Any?> {
    ...
    private lateinit var binding: DialogOrderTimeBinding
            ...
            arguments != null -> {
                when {
                    requireArguments().containsKey(Params.STORE_TIMES_RESPONSE) -> {
                        val storeTimesResponse: StoreTimesResponse =
                            (requireArguments().getSerializable(Params.STORE_TIMES_RESPONSE) as StoreTimesResponse?)!!
