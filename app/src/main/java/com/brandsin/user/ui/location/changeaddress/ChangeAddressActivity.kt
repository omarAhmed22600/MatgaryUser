...
class ChangeAddressActivity : AppCompatActivity(), Observer<Any?>, OnMapReadyCallback
{
    ...
    override fun onCreate(savedInstanceState: Bundle?)
    {
        ...
        observe(viewModel.isMapReady) {
            when (it) {
                true -> {
                    val latLng = LatLng(viewModel.changeAddressRequest.lat!!.toDouble(), viewModel.changeAddressRequest.lng!!.toDouble())
                ...
                when (data) {
                    null -> {
                        ...
                        when (viewModel.changeAddressRequest.lat){
                            null -> {
                                viewModel.changeAddressRequest.lat = PrefMethods.getUserLocation()!!.lat
                                viewModel.changeAddressRequest.lng = PrefMethods.getUserLocation()!!.lng
                                viewModel.changeAddressRequest.streetName = PrefMethods.getUserLocation()!!.address
                            ...
                            data.hasExtra(Params.USER_LOCATION) -> {
                                ...
                                when {
                                    locationItem != null -> {
                                        ...
                                        viewModel.changeAddressRequest.streetName = locationItem.address
